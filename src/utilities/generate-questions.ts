...
async function generateQuestions(followingTweets: FollowingTweets | any, session: User | null) {
  ...
  if (session?.uid) {
    ...
    const unusedTweets = followingTweets.map(({ following, tweets }: FollowingTweets) => ({
      following,
